...
(define (queue-empty? q)
  (and (stream-empty? (queue-left q)) (= (length (queue-right q)) 0))
      )
...
(define (enqueue x q)
  (match q
    [(queue left right size-l size-r)
     (if (= size-l size-r)
         (make-queue (rotate left (cons x right) empty-stream) null (+ size-l size-r 1) 0)
         (make-queue left (cons x right) size-l (+ 1 size-r))
         )
     ]))
...
(define (dequeue q)
  (match q
    [(queue left right size-l size-r)
     (if (= size-l size-r)
         (make-queue (rotate (stream-rest left) right empty-stream) null (+ size-l size-r -1) 0)
         (make-queue (stream-rest left) right (- size-l 1) size-r)
         )
     ]))
